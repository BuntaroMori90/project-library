    ...
    `select e.id as edition_id,e.work_id,w.title as work_title,
       ...
       join lateral (
         select ou.id as owned_id,cu.unit_number,
           coalesce(to_jsonb(ou)->>'custom_cover_url',cu.cover_url,o.custom_cover_url) as cover_url
         from owned_units ou
         ...
         order by cu.unit_number nulls last,ou.id
         limit 1
       ) volume on true
       cross join lateral (select volume.cover_url as value) cover

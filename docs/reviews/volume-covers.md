...
- Il test di scrittura SQL reale e' stato rifiutato dall'approvazione automatica: serve conferma esplicita per eseguirlo. Le prove delle API usano dipendenze simulate; non equivalgono a un salvataggio autenticato end-to-end.

## Rilascio
Applicare migrations/20260917_owned_volume_covers.sql prima di abilitare i salvataggi. Migrazione additiva, gia' applicata solo al branch di test. Produzione non modificata. Le letture restano compatibili con lo schema precedente, le scritture senza colonna restituiscono un messaggio 503 comprensibile. Completare test di salvataggio autorizzati e revisione prima del merge/deploy.
